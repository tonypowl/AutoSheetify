...
export type InstrumentSelectorHandles = {
...
const InstrumentSelector = forwardRef<InstrumentSelectorHandles, InstrumentSelectorProps>(
  ({ selectedInstrument, onInstrumentChange }, ref) => {
    // Optional: Internal state to sync with parent if needed later
    const [localInstrument, setLocalInstrument] = useState<'piano' | 'guitar'>(selectedInstrument);

    useEffect(() => {
      setLocalInstrument(selectedInstrument);
    }, [selectedInstrument]);

    // Expose selectInstrument via ref
